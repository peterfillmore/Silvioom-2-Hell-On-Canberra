...
#define PIN_NUM_RST	 IO_LCD_RESET 
#define PIN_NUM_BCKL     IO_LCD_BACKLIGHT 
...
static const uint8_t
  ...
  Rcmd1[] = {				  // Init for 7735R, part 1 (red or green tab)
	...
	ST7735_VMCTR1 , 1	   ,  // 12: Power control, 1 arg, no delay:
	  0x0E,
	ST77XX_INVOFF , 0	   ,  // 13: Don't invert display, no args, no delay  (Note: display shows inverted - re-invert?)
...
//	  0x00, 0x9F },			  //	 XEND = 159
  ...
  Rcmd3[] = {				  // Init for 7735R, part 3 (red or green tab)
   ...
   rotatecmd[] = { 1,
	   ST77XX_MADCTL, 1, ST7735_MADCTL_BGR 
   };
...
void lcd_init_controller(spi_device_handle_t spi) {
	...
	send_cmd_list(spi, Rcmd2red);
	send_cmd_list(spi, Rcmd3);
	send_cmd_list(spi, rotatecmd);
	
	//uint8_t madctl = 0;
	//madctl = ST77XX_MADCTL_MY | ST77XX_MADCTL_MV | ST77XX_MADCTL_RGB;
	//lcd_cmd(spi, ST77XX_MADCTL);	
...
static void send_lines(spi_device_handle_t spi, int ypos, uint16_t *linedata) {
	...
	trans[1].tx_data[2]=(127+ox)>>8;		//End Col High
	trans[1].tx_data[3]=(127+ox)&0xff;		//End Col Low
...
void lcd_render_fb(uint8_t *fb) {
	...
	for (int yy=0; yy<160; yy+=PARALLEL_LINES) {
		...
		for (int y=0; y<PARALLEL_LINES; y++) {
			...
			int ef_y=((160*(yy+y))/y_sz);
			uint8_t *fbline=&fb[ef_y*240];
			if (ef_y>=160 || ef_y<0) {
				for (int x=0; x<128; x++) *p++=0;
			} else {
				for (int x=0; x<128; x++) {
					*p++=lcdpal[*fbline++];
...
void lcd_init() {
	...
	line_a=calloc(PARALLEL_LINES, 128*2);
	line_b=calloc(PARALLEL_LINES, 128*2);
	assert(line_a);
	assert(line_b);
	...
	for (int y=PARALLEL_LINES; y<128; y+=PARALLEL_LINES) {
		send_line_finish(spi);
