...
void I_ProcessKeyEvents() {
	...
	while (input_get_event(&touchev)) {
		if(touchev.type == EVENT_KEY_START_DOWN) {
			ev.data1 = KEYD_START;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_START_UP) {
			ev.data1 = KEYD_START;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_A_DOWN) {
			ev.data1 = KEYD_A;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_A_UP) {
			ev.data1 = KEYD_A;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		
		}	
		else if(touchev.type == EVENT_KEY_B_DOWN) {
			ev.data1 = KEYD_B;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_B_UP) {
			ev.data1 = KEYD_B;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		
		}else if(touchev.type == EVENT_KEY_UP_DOWN) {
			ev.data1 = KEYD_UP;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_L_UP) {
			ev.data1 = KEYD_L;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		
		}	
		else if(touchev.type == EVENT_KEY_L_DOWN) {
			ev.data1 = KEYD_L;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}else if(touchev.type == EVENT_KEY_R_UP) {
			ev.data1 = KEYD_R;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		
		}	
		else if(touchev.type == EVENT_KEY_R_DOWN) {
			ev.data1 = KEYD_R;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}else if(touchev.type == EVENT_KEY_UP_UP) {
			ev.data1 = KEYD_UP;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_RIGHT_DOWN) {
			ev.data1 = KEYD_RIGHT;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_RIGHT_UP) {
			ev.data1 = KEYD_RIGHT;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		}
		else if(touchev.type == EVENT_KEY_DOWN_DOWN) {
			ev.data1 = KEYD_DOWN;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_DOWN_UP) {
			ev.data1 = KEYD_DOWN;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		}
		else if(touchev.type == EVENT_KEY_LEFT_DOWN) {
			ev.data1 = KEYD_LEFT;
			ev.type = ev_keydown;
			D_PostEvent(&ev);		
		}
		else if(touchev.type == EVENT_KEY_LEFT_UP) {
			ev.data1 = KEYD_LEFT;
			ev.type = ev_keyup;
			D_PostEvent(&ev);		}
		//else if(touchev.type == EVENT_KEY_UP_DOWN) {
